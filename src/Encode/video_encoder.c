...
void set_diemsions(int w, int h, AVRational* fps)
{
    width = w;
    height = h;
    frame_rate = fps;
...
static AVFrame *get_video_frame(OutputStream *ost)
{
    ...
    if (c->pix_fmt != AV_PIX_FMT_YUV420P) {
        ...
        printf("HUGE PROBLEMO\n");
        //avpicture_fill((AVPicture*) ost->frame, data, AV_PIX_FMT_RGB0, c->width, c->height);
...
void set_encode_framebuffer(const uint8_t* data)
{
    rgb_data = data;
}
...
int finish_encoding(void)
{
    ...
    if (enc_inf.have_audio)
        close_stream(enc_inf.oc, &enc_inf.audio_st);
